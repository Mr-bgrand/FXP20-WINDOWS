...
import { Radio, Search, FileSpreadsheet, FileJson, Binary, Type, Copy, Check } from 'lucide-react';
...
export function TagTable({ tags }: TagTableProps) {
  ...
  const filteredTags = useMemo(() => {
    if (!filter) return tagArray;
    const lower = filter.toLowerCase();
    return tagArray.filter((tag) => tag.epc.toLowerCase().includes(lower));
  }, [tagArray, filter]);
  ...
  const getRssiClass = (rssi: number | undefined): string => {
    ...
    if (rssi >= -40) return 'rssi-strong';
    if (rssi >= -55) return 'rssi-medium';
    return 'rssi-weak';
  };
